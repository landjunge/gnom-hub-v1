...
def test_html_full_page_plan_single_worker():
    ...
    assert any(x in tasks[0][1] for x in ("ONE complete", "self-contained single-file"))
    assert "Dark theme" in tasks[0][1]
    assert "binding" in tasks[0][1].lower() or "Definition of Done" in tasks[0][1]
    assert "never truncate" in tasks[0][1].lower() or "</html>" in tasks[0][1].lower()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("onepager coffee shop", None, 3, 99, "onepager"),
        ("mach eine seite für den laden", None, 3, 99, "DE mach eine"),
        ("webseite bauen dark theme", None, 3, 99, "DE webseite bauen"),
        ("create a page for my brand", None, 3, 99, "create a page"),
        # Starke Negative
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("onepager for coffee", None, True),
        ("mach eine seite für den shop", None, True),
        ("webseite bauen", None, True),
        ("", None, False),
        ("page", None, False),
        ("seite", None, False),
