...
5. Free models only when user enables them; budget protection on.
6. One global Save button only.

## Current pipeline target

```
Chat → Brainstorm → Distillation → Coordinator → Worker(s) → Box 3 + Memory
```
